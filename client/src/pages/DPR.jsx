  ...
  const handleSiteSelect = (siteId) => {
    ...
    if (siteId) {
      api.get(`/dpr/sites/${siteId}/po-items`).then(r => {
        ...
        // Auto-populate work items from PO items
        if (r.data.length > 0) {
    ...
    <div className="space-y-6">
      ...
      {tab === 'dashboard' && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="card text-center border-l-4 border-emerald-500"><div className="text-3xl font-bold text-emerald-600">{summary.todaySubmissions}</div><div className="text-sm text-gray-500">DPR Submitted Today</div></div>
          ...
          {summary.missingSites.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2"><FiAlertTriangle className="text-red-600" size={20} /><h4 className="font-bold text-red-700">NO DPR SUBMITTED - Payment Blocked!</h4></div>
              <p className="text-sm text-red-600 mb-3">These sites have NOT submitted DPR today.</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                {summary.missingSites.map(s => (
                  <div key={s.id} className="bg-white border border-red-300 rounded-lg p-3 flex items-center gap-2">
                    <FiMapPin className="text-red-500" /><div><div className="font-medium text-sm">{s.name}</div><div className="text-xs text-gray-500">Supervisor: {s.supervisor || 'N/A'}</div></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {summary.missingSites.length === 0 && summary.activeSites > 0 && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center gap-3">
              <FiCheck className="text-emerald-600" size={24} /><div><h4 className="font-bold text-emerald-700">All sites have submitted DPR today!</h4><p className="text-sm text-emerald-600">Payment processing can proceed.</p></div>
            </div>
          )}
        </>
      )}
        ...
        <>
          ...
          <div className="card p-0 overflow-hidden"><table>
            ...
            <tbody>
              {dprs.map(d => (
                <tr key={d.id}>
                  <td className="font-medium">{d.site_name}</td><td>{d.report_date}</td><td>{d.submitted_by_name}</td><td className="capitalize">{d.weather}</td>
      ...
      {tab === 'sites' && (
        <>
          ...
          <div className="card p-0 overflow-hidden"><table>
            ...
            <tbody>
              {sites.map(s => (<tr key={s.id}><td className="font-medium">{s.name}</td><td>{s.address}</td><td>{s.client_name}</td><td>{s.engineer_name}</td><td>{s.supervisor}</td><td><StatusBadge status={s.status} /></td></tr>))}
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Submit Daily Progress Report" wide>
        <form onSubmit={submitDpr} className="space-y-4 max-h-[75vh] overflow-y-auto pr-1">
          ...
          {/* Work Progress - from PO items */}
          <div className="border rounded-lg p-3 bg-blue-50">
            <h5 className="font-semibold text-sm mb-2">Work Progress (From PO Items)</h5>
            {poItemsForSite.length > 0 ? (
              <>
                <p className="text-xs text-blue-600 mb-2 font-medium">Items auto-loaded from PO. Enter Planned, Actual & Cumulative qty.</p>
                <div className="grid grid-cols-12 gap-2 text-xs font-semibold text-gray-500 mb-1 px-1">
                  <div className="col-span-3">Item Name</div><div>Unit</div><div>BOQ Qty</div><div>Rate</div><div>Amount</div><div>Planned</div><div>Actual</div><div>Cumulative</div><div className="col-span-2">Variance</div>
                </div>
                {workItems.map((w, i) => {
                  const variance = w.planned_qty > 0 ? Math.round(((w.actual_qty - w.planned_qty) / w.planned_qty) * 100) : 0;
                  return (
                    <div key={i} className="grid grid-cols-12 gap-2 mb-1.5 items-center">
                      <div className="col-span-3 text-sm font-medium text-gray-800 truncate" title={w.description}>{w.description}</div>
                      <div className="text-xs text-gray-500">{w.unit}</div>
                      <div className="text-xs font-semibold">{w.boq_qty}</div>
                      <div className="text-xs text-gray-600">Rs {(w.rate || 0).toLocaleString()}</div>
                      <div className="text-xs font-semibold">Rs {(w.amount || 0).toLocaleString()}</div>
                      <input className="input text-sm" type="number" placeholder="0" value={w.planned_qty || ''} onChange={e => { const n = [...workItems]; n[i].planned_qty = +e.target.value; setWorkItems(n); }} />
                      <input className="input text-sm" type="number" placeholder="0" value={w.actual_qty || ''} onChange={e => { const n = [...workItems]; n[i].actual_qty = +e.target.value; setWorkItems(n); }} />
                      <input className="input text-sm" type="number" placeholder="0" value={w.cumulative_qty || ''} onChange={e => { const n = [...workItems]; n[i].cumulative_qty = +e.target.value; setWorkItems(n); }} />
                      <div className={`col-span-2 text-xs font-bold ${variance < 0 ? 'text-red-600' : variance > 0 ? 'text-emerald-600' : 'text-gray-400'}`}>
              ...
              <>
                <p className="text-xs text-amber-600 mb-2">No PO items found for this site. Enter manually.</p>
                {workItems.map((w, i) => (
          ...
          <div className="border rounded-lg p-3 bg-purple-50">
            ...
            {poItemsForSite.length > 0 ? (
              <>
                <p className="text-xs text-purple-600 mb-2 font-medium">Materials auto-loaded from PO items. Enter consumed today & cumulative.</p>
                <div className="grid grid-cols-6 gap-2 text-xs font-semibold text-gray-500 mb-1 px-1">
                  <div className="col-span-2">Material Name</div><div>Unit</div><div>BOQ Qty</div><div>Today</div><div>Cumulative</div>
                </div>
                {materials.map((m, i) => (
                  <div key={i} className="grid grid-cols-6 gap-2 mb-1.5 items-center">
                    <div className="col-span-2 text-sm font-medium text-gray-800 truncate" title={m.material_name}>{m.material_name}</div>
                    <div className="text-xs text-gray-500">{m.unit}</div>
                    <div className="text-xs font-semibold">{m.boq_qty}</div>
                    <input className="input text-sm" type="number" placeholder="0" value={m.consumed_today || ''} onChange={e => { const n = [...materials]; n[i].consumed_today = +e.target.value; setMaterials(n); }} />
                    <input className="input text-sm" type="number" placeholder="0" value={m.cumulative_consumed || ''} onChange={e => { const n = [...materials]; n[i].cumulative_consumed = +e.target.value; setMaterials(n); }} />
                  </div>
                ))}
              </>
            ) : (
              <>
                <p className="text-xs text-amber-600 mb-2">No PO items. Enter manually.</p>
                {materials.map((m, i) => (
        ...
        {selectedDpr && (
          <div className="space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            {selectedDpr.work_items?.length > 0 && (
              <div><h5 className="font-semibold text-sm mb-2">Work Progress</h5><table className="text-xs"><thead><tr><th>Item</th><th>BOQ</th><th>Planned</th><th>Actual</th><th>Cumulative</th><th>Variance</th></tr></thead>
                <tbody>{selectedDpr.work_items.map(w => (<tr key={w.id}><td>{w.description}</td><td>{w.boq_qty}</td><td>{w.planned_qty}</td><td>{w.actual_qty}</td><td>{w.cumulative_qty}</td><td className={w.variance_pct < 0 ? 'text-red-600' : 'text-emerald-600'}>{w.variance_pct}%</td></tr>))}</tbody></table></div>
            )}
            {selectedDpr.manpower?.length > 0 && (
              <div><h5 className="font-semibold text-sm mb-2">Manpower</h5><table className="text-xs"><thead><tr><th>Category</th><th>Required</th><th>Deployed</th><th>Shortage</th></tr></thead>
                <tbody>{selectedDpr.manpower.map(m => (<tr key={m.id}><td>{m.category}</td><td>{m.required}</td><td>{m.deployed}</td><td className={m.shortage > 0 ? 'text-red-600 font-bold' : ''}>{m.shortage}</td></tr>))}</tbody></table></div>
            )}
            {selectedDpr.materials?.length > 0 && (
              <div><h5 className="font-semibold text-sm mb-2">Material Consumption</h5><table className="text-xs"><thead><tr><th>Material</th><th>BOQ Qty</th><th>Today</th><th>Cumulative</th><th>Balance</th></tr></thead>
